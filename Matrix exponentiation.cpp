...
problem: given a matrix A of n*n and an integer k;
find the value of A+A^2+A^3+.....+A^k;
...
struct matrix
{
    ...
    matrix operator + (const matrix &p)const
    {
        ...
        for(int i=0; i<r; i++)
            for(int j=0; j<c; j++)
                res.mat[i][j]=(mat[i][j]+p.mat[i][j])%10;
        return res;
    }
